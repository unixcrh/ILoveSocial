...
-(id)initWithImage:(UIImage*)image userID:(NSString*)userID photoID:(NSString*)photoID
{
    ...
    _photoID=[[NSString alloc] initWithString:photoID];
            ...
            if (!client.hasError) {
                ...
                for(NSDictionary *dict in array) {
                 //   NSLog(@"%@",dict);
                    ...
                    Image  * image = [Image imageWithURL:bigPhoto inManagedObjectContext:_context];
                    if (!image)
                    {
                        [UIImage loadImageFromURL:bigPhoto completion:^{
                            Image *image1 = [Image imageWithURL:bigPhoto inManagedObjectContext:_context];
                            [_imageView setImage:[UIImage imageWithData:image1.imageData.data]];
